        ...
        : TForm(Owner)
{
  ...
  if (!LoadHtmlHelp())
  {
    ShowMessage("HTML Help was not detected on this computer. EASyCF help may not work correctly.");
  }
  ...
  try {
    memory = new char[MEMSIZE];      // reserve coldfire memory space
    for (int i=0; i<MEMSIZE; i++)
      memory[i] = 0xFF;              // erase coldfire memory to $FF
  } catch(...){
    Application->MessageBox("Error reserving main Coldfire memory in Form1::FormCreate",NULL,mbOK);
...
__fastcall TForm1::~TForm1()
{
  try{
    if (memory != NULL)
      delete[] memory;            // free coldfire memory space
...
//---------------------------------------------------------------------------
// This is the function that runs the coldfire program
void __fastcall TForm1::runLoop()
{
  ...
  try {
    while(runMode){
      runprog();          // run next coldfire instruction
  ...
  try {
    ...
    if(loadSrec(name.c_str()) == false) {  // if S-Record loads with no errors
      ...
      if (FileExists(str)) {
        ...
        // scan listing file for *[simcf] commands
        // A comment line that has the following form will
        // automatically set an option in SimCF
        //
        // *[simcf]break      sets a PC breakpoint on the following line of code:
        // *[simcf]bitfield   enables support for bitfield instructions
        //
        for (int i=0; i<ListBox1->Items->Count; i++) {
          str = ListBox1->Items->Strings[i].LowerCase();
          if (str.SubString(41,14) == "*[simcf]break") { // if break set
          ...
          if (str.SubString(41,17) == "*[simcf]bitfield") { // if bitfield enabled
            bitfield = true;
        ...
        ListBox1->Items->Add("");
        ListBox1->Items->Add("Use EASyCF to assemble the source file to create");
...
// saves simulator settings to file
{
  try {
    AnsiString str;
    str = ExtractFilePath(Application->ExeName) + "simCF.dat";
    char fileName[256];
    strcpy(fileName, str.c_str());        // fileName is path + simCF.dat
    ...
    MessageDlg("Error saving SimCF settings",mtInformation, TMsgDlgButtons() << mbOK,0);
    return;
  }
}
...
void __fastcall TForm1::defaultSettings()
{
  try {
    ...
    simIO->Top = 100;                 //'SimIO Form Top' setting
    simIO->Left = 400;                //'SimIO Form Left' setting
    simIO->Visible = true;            //'simIO Visible'
    MemoryFrm->Top = 80;              //'Memory Form Top' setting
    MemoryFrm->Left = 280;            //'Memory Form Left' setting
    StackFrm->Top = 200;              //'Stack Form Top' setting
    StackFrm->Left = 40;              //'Stack Form Left' setting
    StackFrm->Height = 538;           //'Stack Form Height' setting
    ...
    MessageDlg("Error setting default SimCF settings",mtInformation, TMsgDlgButtons() << mbOK,0);
    return;
  }
}
...
// If the contents of simCF.dat to not match expected then default values
// are used.
void __fastcall TForm1::LoadSettings()
{
  ...
  Form1->BitFieldEnabled->Checked = bitfield;
    ...
    MessageDlg("Error loading SimCF settings",mtInformation, TMsgDlgButtons() << mbOK,0);
    return;
  }
}
//---------------------------------------------------------------------------
