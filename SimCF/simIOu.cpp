...
extern bool disableKeyCommands;  // defined in SIMCFu
        ...
        *inputCh = Key;         // get key
        ...
        if(inputNumber == NULL)         // if string input
        {
          ...
          *inputLength = keyI;            // length of input string to D1
          ...
          strcpy(inputBuf,keyBuf);        // copy to simCF inputBuf
          *inputLength = keyI;            // length of input string to D1
...
// Load a WAV file into DirectX sound memory (not coldfire memory).
...
{
  int size = (uint)(settings & 0x0000FFFF);     // number of bytes to send
...
{
  int size = (uint)(settings & 0x0000FFFF);     // number of bytes to read
