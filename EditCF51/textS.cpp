...
{
  const int DIRECTIVE_COUNT = 29;
  static char *directives[] = {"DC","DCB","DS","END","EQU","FAIL","INCBIN",
                               "INCLUDE","MACRO","ENDM","MEMORY","OFFSET","OPT","ORG",
                               "REG","SECTION","SET","IFC","IFNC","IFEQ","IFNE",
                               "IFLT","IFLE","IFGT","IFGE","ENDC","FAIL","MEXIT","NARG"};
  ...
  try {
    ...
    for(int i=0; i<DIRECTIVE_COUNT; i++) {      // for all directives
      if( !(strcmp(tokenS[1], directives[i])) ) {  // if match
        ...
        if( (strcmp(tokenS[1], "ENDM")) ){      // if not ENDM
