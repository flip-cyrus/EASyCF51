  ...
  if(PrintDialog->Execute())
  {
    try //try to print the text
    {
      ...
      if (EditorOptionsForm->PrintBlack->Checked) {
        //Create temp TTextStuff
        TTextStuff *RichPrint = new TTextStuff(Application);
        ...
        RichPrint->SourceText->PageRect =
                  Rect(100,100,Prntr->PageWidth - 100, Prntr->PageHeight - 100);
        RichPrint->SourceText->Print(Active->Project.CurrentFile);
        delete RichPrint;
      } else {          // print in color
        Active->SourceText->PageRect =
                  Rect(100,100,Prntr->PageWidth - 100, Prntr->PageHeight - 100);
        Active->SourceText->Print(Active->Project.CurrentFile);
      }
      delete Prntr;
    }
...
void __fastcall TMain::tbPrintClick(TObject *Sender)
{
 try //try to print the text
 {
    //Grab the active Child Form
    ...
    TPrinter *Prntr = new TPrinter;
    Active->SourceText->PageRect = Rect(100,100,Prntr->PageWidth - 100, Prntr->PageHeight - 100);
    Active->SourceText->Print(Active->Project.CurrentFile);
    delete Prntr;
  }
  catch(...) //if there is an error then cancel print
